...
function initMap() {
    ...
    // Check if the KML URL is defined and not empty
    // ... (your existing initMap code)


    // Function to parse KML data and display it on the map
    function parseKmlData(kmlData) {
        // Loop through each feature in the KML data
        kmlData.forEach(function(feature) {
            // Check the type of feature
            if (feature.getType() === 'KmlPlacemark') {
                // It's a placemark, extract its geometry and properties
                const placemark = feature.getGeometry();
                const placemarkName = feature.getName();
                const placemarkDescription = feature.getDescription();
                ...
                const marker = new google.maps.Marker({
                    ...
                    map: map,
                    title: placemarkName,
                    // You can customize the marker icon, color, etc. here
                });
            } else if (feature.getType() === 'KmlPolygon') {
                // It's a polygon, extract its geometry and properties
                const polygon = feature.getGeometry();
                const polygonName = feature.getName();
                const polygonDescription = feature.getDescription();

                // Create an array to hold polygon coordinates
                const polyCoords = [];

                // Loop through each coordinate of the polygon
                polygon.getArray().forEach(function(latlng) {
                    polyCoords.push({
                        lat: latlng.lat(),
                        lng: latlng.lng()
                    });
                ...
                const poly = new google.maps.Polygon({
                    ...
                    // You can customize polygon color, fill opacity, etc. here
                });
            }
            // Add more conditions for other types of features as needed
        });
    }
    ...
    datadistricts.forEach((location, _index) => {
    ...
    dataLocation.forEach((location, _index) => {
        let lat = location.getAttribute("data-lat")
        let long = location.getAttribute("data-lng")
        let loc = location.getAttribute("data-location")
    ...
    dataBoundary.forEach((location, _index) => {
    ...
    dataParcel.forEach((parcel, _index) => {
    ...
    // Add an event listener for when an overlay is complete
    // kml parsing per inserted to display in the map
    // Load KML files from the server
    // Load KML files from the server
    kmlFiles.forEach(function(src) {
        const kmlLayer = new google.maps.KmlLayer({
            url: src,
            map: map,
        ...
        // Add a console log to confirm the KML file is being loaded
        console.log('Loading KML file:', src);

        // Add a listener to check if the KML layer is displayed successfully
        google.maps.event.addListenerOnce(kmlLayer, 'defaultviewport_changed', function() {
            console.log('KML file displayed:', src);
        });

        google.maps.event.addListener(kmlLayer, 'status_changed', function() {
            if (kmlLayer.getStatus() !== google.maps.KmlLayerStatus.OK) {
                console.error('Error loading KML file:', src, kmlLayer.getStatus());
            }
        });
    });
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            if (file.name.toLowerCase().endsWith('.kml')) {
                ...
                console.log('KMZ file uploaded:', file.name);
                displayKMZ(fileContent);
            } else {
...
function displayKML(kmlData) {
    ...
    // Add a console log to confirm the KML file is being displayed
    google.maps.event.addListenerOnce(kmlLayer, 'defaultviewport_changed', function() {
        console.log('KML data displayed on map');
    });

    google.maps.event.addListener(kmlLayer, 'status_changed', function() {
        if (kmlLayer.getStatus() !== google.maps.KmlLayerStatus.OK) {
            console.error('Error displaying KML data on map:', kmlLayer.getStatus());
        }
    });
}
...
function displayKMZ(kmzData) {
    JSZip.loadAsync(kmzData).then(function(zip) {
        ...
        console.error('Failed to parse KMZ file:', err);
    });
